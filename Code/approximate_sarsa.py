...
class approximate_sarsa_agent(object):
    ...
    def phi(self, state, action, demand=None):
    ...
    def get_action(self, state, demand=None):
        ...
        if demand:
            return actions[np.argmax(np.dot(self.theta, self.phi(state, actions, demand)))]
    ...
    def update(self, state, action, reward, state_new, action_new):
        # Calculate delta
        delta = reward + self.env.gamma * np.dot(self.theta, self.phi(state_new, action_new)) - np.dot(self.theta,
                                                                                                       self.phi(state,
                                                                                                                action))
