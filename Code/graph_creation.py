...
add_reinforce_1 and add_graph_to_show("reinforce", "REINFORCE - Linear")
add_reinforce_2 and add_graph_to_show("reinforce_phi2", "REINFORCE - Quadratic")
add_reinforce_3 and add_graph_to_show("reinforce_phi3", "REINFORCE - RBF")
for test_num in range(len(tests)):
    ...
    if tests_to_graph[test_num]:
        ...
        for agent_num in range(len(agents)):
            ...
            rewards = pd.read_csv(results_folder_path + test + "_" + agent + "_rewards.csv",
        ...
        if False:
            ...
            for agent_num in range(len(agents)):
                agent = agents[agent_num]
                best_actions = pd.read_csv(results_folder_path + test + "_" + agent + "_best_actions.csv",
                                           header=None).values
        ...
        if True:
            for agent_num in range(len(agents)):
                ...
                agent = agents[agent_num]
                print(results_folder_path + test + "_" + agent + "_best_stocks.csv")

                best_stocks = pd.read_csv(results_folder_path + test + "_" + agent + "_best_stocks.csv",
                                          header=None).values
                print(len(best_stocks))
